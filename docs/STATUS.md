...
2. M4 に着手(2026-07-05 に再定義済み): M4a ウォームアップフラグ → M4b 週間ビュー(サマリ・基準週切替・種目別内訳+タグフィルタ・週次推移バー)→ M4c 折れ線グラフ → M4d MAX。集計ルールは requirements.md「分析」節に確定済み(ウォームアップ・実績空欄は除外、自重は体重+加重・体重未登録は除外+注記、週は月曜始まり)。自重の負荷計算は lib/setFormat.ts の effectiveLoad を使う
