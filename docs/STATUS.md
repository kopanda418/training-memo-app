...
- **フェーズ**: 🎉 Ver 1.0.3 リリース済み(2026-07-07)。追加仕様+バグ修正 8 項目のうち実装 7 件を全対応、調査 1 件(#8)完了
- **作業中マイルストーン**: なし。以降はバックログ(roadmap.md)とユーザーフィードバックに基づく継続開発

## 次にやること

1. **#1 暗転バグ・#8 タグ消失の実機観察**: v1.0.3 の #1 対策(ErrorBoundary + ヘッダー不透過化)後も暗転/タグ消失が起きるか確認する。続く場合は次項へ
2. **(#8 続行時のみ)書き込みエラーの可視化**: `changeBlockTag` 等の fire-and-forget 書き込みに `.catch(() => showToast(...))` を付け、無言の保存失敗を捕まえる(詳細は `docs/investigation-tag-loss.md` の仮説B)
